 ...
 * Plugin URI:        bigideas
 * Description:       Allows a user to post an idea to an Ideas page at /Ideas/. A BuddyPress group with bbPress forum are automatically created when this post is published.
 * Version:           1.0.0
 * Author:            WhatsTheBigIdea
 * Author URI:        https://github.com/whatsthebigideas/bigideas
 * License:           GPL-2.0+
 ...
 */
function ideas_deactivate_ideas()
{
    require_once plugin_dir_path(__FILE__) . 'includes/class-ideas-deactivator.php';
    Ideas_Deactivator::deactivate();
}

register_activation_hook(__FILE__, 'ideas_activate_ideas');
register_deactivation_hook(__FILE__, 'ideas_deactivate_ideas');
 ...
 */
add_filter('pre_get_posts', 'ideas_exclude_ideas_category');

function ideas_exclude_ideas_category($query)
{
...
function ideas_testing($an_array)
{
     ...
     */
    $file = plugin_dir_path(__FILE__) . "/testing/ideas_testing.txt";

    $text = "########################################################################" . PHP_EOL;
    $text .= "ABSPATH: " . ABSPATH . PHP_EOL;
    $text .= "ABSPATH . ideas.xml: " . ABSPATH . "ideas.xml" . PHP_EOL;
    $text .= "plugin_dir_path(__FILE__): " . plugin_dir_path(__FILE__) . PHP_EOL;
    $text .= "plugins_url('ideas/js/ideas.js'): " . plugins_url('ideas/js/ideas.js') . PHP_EOL;
    $text .= "get_stylesheet_directory_uri(): " . get_stylesheet_directory_uri() . PHP_EOL;
    $text .= "get_template_directory_uri(): " . get_template_directory_uri() . PHP_EOL;
    $text .= "get_site_url() . '/ideas.xml': " . get_site_url() . '/ideas.xml' . PHP_EOL;
    $text .= "########################################################################" . PHP_EOL;
 ...
 * *****************************************************************************************************************
 */
add_action('publish_post', 'ideas_create_ideas_xml', 10, 2);

function ideas_create_ideas_xml()
{
...
function ideas_sxml_append(SimpleXMLElement $to, SimpleXMLElement $from)
{
    ...
    if (has_category('ideas', $post)) {
        ...
        for ($x = 0; $x < $array_length; $x ++) {
            ...
            if (! empty($domains_array[$x])) {
                ...
                $response = wp_remote_get($domain_xml);
                ...
                $outside_simplexml_object = simplexml_load_string($body);
